from dto.OutputModel import OnePieceCharacters
from fastapi import (APIRouter, Path)
from asyncio import gather
from use_case.Actions import FindById
...
router = APIRouter(prefix='/op', tags=['One-Piece'])

@router.get('/getall', response_model=OnePieceCharacters)
async def GetAll():
...
@router.get('/getbyid/{id}', response_model=OnePieceCharacters)
async def GetById(id: int = Path(gt=0, le=9)):
    gather_result = await gather(FindById(id=id))
    print(type(gather_result))
    # return gather_result[0]
    return OnePieceCharacters(data=gather_result[0])
